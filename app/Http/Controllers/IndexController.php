...
class IndexController extends Controller
{
    ...
    public function locphim(){
        ...
        if($order=='' && $genre_get=='' && $country_get=='' && $year_get==''){
            ...
            $meta_description = "Lọc theo tiêu đề";
            $meta_image = "";

            //lấy dữ liệu
            $movie_array = Movie::withCount('episode'); //lấy ra phim và số tập
    ...
    public function genre($slug){
        ...
        if(isset($_GET['0'])){
            $movie = Movie::withCount(['episode'=>function($q) { $q->where('server', 3); }])->where('thuocphim', '0')->whereIn('id', $many_genre)->orderBy('ngaycapnhat', 'DESC')->paginate(40);
        }
        else{
            $movie = Movie::withCount(['episode'=>function($q) {$q->where('server', 3);}])->whereIn('id', $many_genre)->orderBy('ngaycapnhat', 'DESC')->paginate(40);
        }
