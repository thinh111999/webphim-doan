                  ...
                  <article class="thumb grid-item post-38498">
                     <div class="halim-item">
                        <a class="halim-thumb" href="{{route('movie',$hot->slug)}}" title="{{$hot->title}}">
                           ...
                           <figure>
                              <?php
                              // Kiểm tra xem ảnh có trong folder uploads/movie hay không
                              $imagePath = public_path('uploads/movie/' . $hot->image); // Đường dẫn tới ảnh trong folder
                              if (file_exists($imagePath)) {
                                    ?>
                                    <img class="lazy img-responsive" src="{{asset('uploads/movie/'.$hot->image)}}" alt="{{$hot->title}}" title="">
                                    <?php
                              } else {
                                    // Nếu ảnh không có trong folder, sử dụng link HTTP
                                    ?>
                                    <img class="lazy img-responsive" src="{{$hot->image}}" alt="{{$hot->title}}" title="">
                                    <?php
                              }
                              ?>
                           </figure>
                        ...
                        <div class="halim-item">
                           <a class="halim-thumb" href="{{route('movie',$mov->slug)}}">
                              ...
                              <figure>
                                 <?php
                                 // Kiểm tra xem ảnh có trong folder uploads/movie hay không
                                 $imagePath = public_path('uploads/movie/' . $mov->image); // Đường dẫn tới ảnh trong folder
                                 if (file_exists($imagePath)) {
                                 ?>
                                    <img class="lazy img-responsive" src="{{asset('uploads/movie/'.$mov->image)}}" alt="{{$mov->title}}" title="">
                                 <?php
                                 } else {
                                    // Nếu ảnh không có trong folder, sử dụng link HTTP
                                 ?>
                                    <img class="lazy img-responsive" src="{{$mov->image}}" alt="{{$mov->title}}" title="">
                                 <?php
                                 }
                                 ?>
                              </figure>
